...
let client = googleImages('003271711760641036001:je9-wteloym', 'AIzaSyAbCRUiXvYraHY6DAaz69PegJ_w9LyPnEo');
/**
...
HanBot.prototype._onMessage = function (message) {
	if(this._validate(message))
	{
			...
			{
		    	console.log('Response Sent: joke');
		        this._replyWithRandomJoke(message);
		    }
	    	else if( this._isDirectMessage(message))
	    	{
		    	if(message.text.toLowerCase().indexOf('pin') > -1) 
		    	{
		    		this._pinResponse(message);
		    	}
		    	if(message.text.toLowerCase().indexOf('snack') > -1) 
		    	{
		    		this._snack(message);
		    	}
		    	if(message.text.toLowerCase().indexOf('bandito') > -1) 
		    	{
		    		this._bandito(message);
		    	}
		    	else
		    	{
			    	console.log('Response Sent: greeting');
...
HanBot.prototype._pinResponse = function(message) {
...
HanBot.prototype._replyWithGreeting = function (originalMessage) {
    ...
    console.log('No command: Error Response');
 	var response = "I'm Sorry I didn't recognize that could you try something else?";
...
HanBot.prototype._getUserName = function (message) {
    var self = this;


    this.user = this.users.filter(function (user) {
        return user.name === message.user;
    })[0];
    console.log('User is :' + this.user);
};
/**
...
HanBot.prototype._bandito = function (originalMessage) {
    ...
    self.functionsdb.get('SELECT ind, response FROM bandito ORDER BY used ASC, RANDOM() LIMIT 1', function (err, record) {
    	...
    	// self.postMessageToChannel(channel.name, record.response , {as_user: true});
