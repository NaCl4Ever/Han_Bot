...
HanBot.prototype._onMessage = function (message) {
	if(this._validate(message))
	{
		    	...
		    	else if(message.text.toLowerCase().indexOf('bandito') > -1) 
		    	{
...
HanBot.prototype._addlunch = function (originalMessage) {
    ...
    self.functionsdb.get('SELECT * FROM restaurants WHERE name = ? ORDER BY used ASC, RANDOM() LIMIT 1 ', name , function (err, record) {
        ...
        if(record === undefined)
        {
            ...
            self.functionsdb.run("INSERT INTO restaurants (name, type, used, last_visit) VALUES (?, '' , 0, '2013-10-07 08:23:19');", name.toUpperCase());
            self.postMessage(originalMessage.channel, 'I have added the ' + name.toUpperCase() + ' into the database!' , {as_user: true});
        }
        console.log(record)
    });
};
